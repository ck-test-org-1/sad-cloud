...
package policies.tf_network_flow_log_retention
...
security_groups = lacework.resources("azurerm_network_security_group")
flow_logs = lacework.resources("azurerm_network_watcher_flow_log")
...
policy[p] {
  ...
  p = lacework.allow_resource(sg)
} {
  sg = security_groups[_]
  not watched_security_group_ids[lower(sg.id)]
  p = lacework.deny_resource(sg)
}